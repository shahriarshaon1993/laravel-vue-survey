      ...
      <div class="shadow sm:rounded-md sm:overflow-hidden">
        ...
        <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
          ...
          <div>
            ...
            <textarea id="description" name="description" rows="3" v-model="model.description" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md" placeholder="Description" />
          </div>
          <!--/.Description-->
...
<script setup>
  ...
  if (route.currentRoute.value.params.id) {
    model.value = store.state.surveys.find(
      (s) => s.id === parseInt(route.currentRoute.value.params.id)
    );
  }
</script>
