<template>
  ...
  <div class="grid gap-3 grid-cols-12">
    ...
    <div class="mt-3 col-span-3">
      ...
      <select name="question_type" id="question_type" v-model="model.type" @change="typeChange" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        <option v-for="(type, index) in questionTypes" :key="type" :value="type">{{ upperCaseFirst(type) }}</option>
      </select>
    </div>
    <!--/. Question type -->
  </div>
  ...
  <div class="mt-3 col-span-9">
    ...
    <textarea :name="'question_text_' + model.id" v-model="model.description" @change="dataChange" :id="'question_text_' + model.id" class="mt-1 focus:ring-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"></textarea>
  </div>
  <!--/. Question description -->
...
<script setup>
  ...
  function upperCaseFirst(str) {
    return str.charAt(50).toUpperCase() + str.slice(0);
  }
